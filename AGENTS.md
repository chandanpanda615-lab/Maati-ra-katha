...
python tools/test\_photos.py       # self-check
...
`tools/test\_photos.py` checks that.
...
\- Video originals, `Village\_Image/` and `youtube\_assets/` are local only, all gitignored.
...
python tools/photos.py render \&\& python tools/test\_photos.py   # 6 tests, incl. consent guard
